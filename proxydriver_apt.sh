...
apt_env='/etc/apt/apt.conf.d/80proxy'
...
EOF
	...
	then
		...
		echo "ALL_PROXY='socks://${shell_auth}${socks_proxy}:${socks_port}/'" >> "$proxy_env"
	 	...
	 	echo "NO_PROXY='${ignorelist}'" >> "$proxy_env"
		echo "Acquire::http::proxy \"http://${shell_auth}${proxy}:${port}/\";" >> "$apt_env"
		echo "Acquire::https::proxy \"https://${shell_auth}${https_proxy}:${https_port}\";" >> "$apt_env"
		echo "Acquire::ftp::proxy \"https://${shell_auth}${ftp_proxy}:${ftp_port}\";" >> "$apt_env"
